...
const SignUpContainer =() => {
    ...
    const onSubmitForm = async e => {
       ...
       try{
         ...
         const response = await fetch("http://localhost:1000/a/pat",
    ...
    const style={textAlign:"center", fontSize:"30px", fontWeight:'bold'}
        return (
            <div className='loginContainer'>
                <h1 style={style}>Patient Booking Form</h1>
            <form  onSubmit={onSubmitForm}>
                Name <input type="text" id='uname'  placeholder='Patient Name' value={name} onChange={(e)=>setName(e.target.value)} 
              required/><br/>
                Age <input type="text" id='p' placeholder='Age' onChange={(e)=>setAge(e.target.value)} required/><br/>
                Email Address < input type='text' id='email' placeholder='Number' onChange={(e)=>setEmail(e.target.value)} required/>
